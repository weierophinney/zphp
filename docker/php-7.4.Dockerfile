# Build Swoole
FROM cr.zend.com/zendphp/7.4:ubuntu-20.04-cli as swoole
...
RUN set -e; \
    apt-get update; \
    apt-get install -y php7.4-zend-dev libcurl4-openssl-dev; \
    mkdir /workdir; \
    cd /workdir; \
    ...
    phpize7.4-zend; \
    ./configure \
        --with-php-config=/usr/bin/php-config7.4-zend \
        --enable-http2 \
        --enable-openssl \
        --enable-sockets \
        --enable-openswoole \
...
FROM cr.zend.com/zendphp/7.4:ubuntu-20.04-cli
...
## Install Swoole
COPY --from=swoole /usr/lib/php/7.4-zend/openswoole.so /usr/lib/php/7.4-zend/openswoole.so
COPY --from=swoole /usr/include/php/7.4-zend/ext/openswoole /usr/include/php/7.4-zend/ext/openswoole
RUN set -e; \
